...
function ProfileMakerForm({forwardButton, backwardButton}) {
    ...
    const handleFileUpload = (e: EventTarget): void => {
        const file = e.target.files[0];
        handleConversion(file, (base64String: string): void => {
            console.log(base64String)
        }).catch((error) => console.log(error));
    }

    //converts image + compresses to base64-encoded string
    const handleConversion = async (file: File, callback: (base64String: string) => string): Promise<void> => {
        let compressedFile
        const options = {
            maxSizeMB: 1,
